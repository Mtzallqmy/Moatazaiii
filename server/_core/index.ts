...
import { refreshPublicIndex } from "../scheduledRefresh";
...
async function startServer() {
  ...
  app.get("/health", (_req, res) => res.status(200).json({ status: "ok", service: "telegram-public-search" }));
  registerStorageProxy(app); registerOAuthRoutes(app);
  app.post("/api/scheduled/public-index-refresh", async (req, res) => {
    try {
      const user = await sdk.authenticateRequest(req);
      if (!user.isCron || !user.taskUid) return res.status(403).json({ error: "cron-only" });
      const setting = await db.getRefreshSettingByTaskUid(user.taskUid);
      if (!setting || !setting.enabled) return res.json({ ok: true, skipped: "orphan-or-disabled" });
      return res.json(await refreshPublicIndex(setting));
    } catch (error) {
      const details = error instanceof Error ? { error: error.message, stack: error.stack } : { error: "unknown" };
      return res.status(500).json({ ...details, context: { path: req.path }, timestamp: new Date().toISOString() });
    }
  });
